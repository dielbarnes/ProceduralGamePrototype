...
bool ObjModel::InitializeBuffers(ID3D11Device* device, int iInstanceCount, Instance* instances)
{
...
void ObjModel::SetTexture(ID3D11ShaderResourceView &texture)
{
	m_pTexture = &texture;
}

ID3D11ShaderResourceView** ObjModel::GetTexture()
{
	return &m_pTexture;
}

void ObjModel::SetVertexCount(int iCount)
{
	m_iVertexCount = iCount;
}

void ObjModel::SetIndexCount(int iCount)
{
...
void ObjModel::SetVertexData(VertexData* vertexData)
{
	m_vertexData = vertexData;
}

XMMATRIX ObjModel::GetWorldMatrix()
{
	return m_worldMatrix;
}

void ObjModel::TransformWorldMatrix(XMMATRIX translationMatrix, XMMATRIX rotationMatrix, XMMATRIX scalingMatrix)
{
	m_worldMatrix = m_worldMatrix * translationMatrix * rotationMatrix * scalingMatrix;
}

XMFLOAT4 ObjModel::GetAmbientColor()
{
	return m_ambientColor;
}

XMFLOAT4 ObjModel::GetDiffuseColor()
{
	return m_diffuseColor;
}

XMFLOAT4 ObjModel::GetSpecularColor()
{
	return m_specularColor;
}

float ObjModel::GetSpecularPower()
{
	return m_fSpecularPower;
}

void ObjModel::SetLightDirection(float x, float y, float z)
{
	m_lightDirection = XMFLOAT3(x, y, z);
}

XMFLOAT3 ObjModel::GetLightDirection()
{
...
void ObjModel::Render(ID3D11DeviceContext* immediateContext)
{
